...
export function ChessLearn({ section }: { section: "pieces" | "rules" }) {
  ...
  return <section className="chess-panel" aria-labelledby="pieces-title">
    ...
    <div className="chess-piece-tabs" aria-label="Chọn quân cờ">{PIECE_LESSONS.map((item, index) => <button key={item.piece} type="button" aria-pressed={pieceIndex === index} onClick={() => { setPieceIndex(index); setFeedback(""); }}><span aria-hidden="true">{item.symbol}</span><strong>{item.nameVi}</strong><small>{item.nameEn}</small></button>)}</div>
